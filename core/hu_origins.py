...
ORIGIN_RULES: list[tuple[str, Origin]] = [
    ("TH", Origin("THA", "Tailandia (THA)", auto_pallet=False, color="#BDD7EE", phase2_wait=1.0)),
    ("T100", Origin("CNA", "China (CNA)", auto_pallet=False, color="#BDD7EE", phase2_wait=1.0)),
    ("C10",  Origin("BRA/ATL", "ATL, (BRA/ATL)",   auto_pallet=False, color="#C6EFCE", phase2_wait=3.0)),
    ("29",   Origin("ITA", "Italia (ITA)",    auto_pallet=True,  color="#FFEB9C", phase2_wait=2.5)),
    ("ELPS", Origin("FHR", "Foothill Ranch (ELPS)",      auto_pallet=True, color="#F4B084", phase2_wait=3.0)),
]
UNKNOWN_ORIGIN = Origin("UNK", "Desconocido", auto_pallet=False, color="#E2EFDA", phase2_wait=2.0)
