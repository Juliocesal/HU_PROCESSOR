...
# os.path.dirname(__file__) apunta a la carpeta donde está pdf_printer.py,
# así la ruta funciona sin importar desde dónde se ejecute la app.
# ── Después ──────────────────────────────────────────────────────────────────
def _get_base_dir() -> str:
    """Retorna la raíz correcta tanto en desarrollo como en .exe (PyInstaller)."""
    if hasattr(sys, "_MEIPASS"):
        return sys._MEIPASS          # carpeta _internal/ en el .exe
    return os.path.dirname(os.path.abspath(__file__))

SUMATRA_BUNDLED = os.path.join(_get_base_dir(), "vendors", "SumatraPDF.exe")
