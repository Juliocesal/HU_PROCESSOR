...
class PrintDialogWatcher:
    ...
    def start(self):
        if self._thread and self._thread.is_alive():
            ...
            log.debug("print_watcher already running — resetting idle timer")
            return
        
        # Asegurar que el viejo thread está completamente parado antes de crear uno nuevo
        # Esto evita race conditions cuando se reinicia el watcher en segunda ronda
        self._stop_event.set()
        time.sleep(0.05)  # Pequeña pausa para que el viejo thread termine gracefully
        
        # Ahora crear uno nuevo limpio
        self._confirmed_count = 0
        self._last_hwnd       = 0
        self._thread = threading.Thread(target=self._run, daemon=True)
        self._thread.start()
        log.info("print_watcher_started idle_timeout=%.1fs", self._idle_timeout)
    ...
    def _run(self):
        # Sincronizar: limpiar el evento al iniciar (por si acaso estaba SET)
        self._stop_event.clear()
