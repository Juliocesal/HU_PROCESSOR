...
function handleInitialState(msg) {
  ...
  if (msg.items && msg.items.length > 0) {
    ...
    tbody.innerHTML =
      `<tr class="empty-row" id="empty-row"><td colspan="5">Sin HUs en cola - escanea el primer código</td></tr>`;
  }
...
function handleStatsUpdate(msg) {
  ...
  document.getElementById('kpi-err').textContent   = stats.errors;
  document.getElementById('kpi-pend').textContent  = stats.pending;
  document.getElementById('pal-lbl').textContent = `Pallets activos: ${stats.pallets}`;
  document.querySelector('.kpi-card.kpi-err').classList.toggle('has-errors', stats.errors > 0);
  updateProgress();
  updateButtons();
}
...
function deleteButtonHTML(huCode, status) {
  return '';
}
...
function buildRowHTMLLegacy(item) {
...
function ensurePalletSep(palletId, originCode) {
  ...
  sep.innerHTML = `<td colspan="5" id="sep-${palletId}">-- Pallet P${pid} - ${originCode || 'Sin origen'} - 0 HUs --</td>`;
  palletSepRows[palletId] = sep;
  tbody.appendChild(sep);
  _observeSep(sep);   //  AGREGAR ESTA LNEA
}
...
function updatePhaseCardsLegacy() {
  ['f1','f2'].forEach(id => {
...
function setHintLegacy(msg, type) {
...
// 
//  5-HREPROCESAR - falta en el HTML original
