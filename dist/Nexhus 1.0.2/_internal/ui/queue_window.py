...
class PhaseCard(WhiteCard):
    ...
    def __init__(self, code: str, tcode: str, desc: str, parent=None):
        ...
        image: url("{resource_path('ui/icons/check.svg')}");
        }}
...
class ShellBar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Avatar - extract initials from Windows username
        username = os.getenv("USERNAME", "USER").upper()
        if "." in username:
            parts = username.split(".")
            avatar_text = (parts[0][0] if parts[0] else "") + (parts[1][0] if len(parts) > 1 and parts[1] else "")
        else:
            avatar_text = username[:2]
        self._av = QLabel(avatar_text)
...
class FooterBar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.btn_reprocess = _foot_btn("⟲  REPROCESAR",  "ghost")
        h.addWidget(self.btn_reprocess)
...
class QueueWindow(QMainWindow):
    ...
    def _make_scan_card(self) -> WhiteCard:
        ...
        ico.setStyleSheet(f"""
            image: url('{resource_path('ui/icons/scancode.svg')}');
            background: transparent;
        """)
    ...
    def _on_scan(self):
        ...
        if not (10 <= len(raw) <= 15):
            self._flash(
            f"⚠ Código HU inválido: '{raw}' tiene {len(raw)} caracteres. Rango permitido: 10–15",
        C.ORANGE
            )
            return
    ...
    def _on_table_context_menu(self, position):
        ...
        # ¿Es separador de pallet?
        pallet_id_clicked = None
        for pid, sep_row in self._pallet_sep_rows.items():
            if sep_row == row:
                pallet_id_clicked = pid
                break

        if pallet_id_clicked is not None:
            # Es un separador — verificar si el pallet es seguro borrar
            if self._queue.pallet_is_safe_to_delete(pallet_id_clicked):
                delete_action = menu.addAction("🗑 Borrar pallet completo")
            else:
                action_disabled = menu.addAction("⛔ Pallet en proceso — no se puede borrar")
                action_disabled.setEnabled(False)
                delete_action = None
        else:
            # Es una fila de HU — verificar status individual
            hu_item = self.table.item(row, 2)
            hu_status = None
            if hu_item:
                hu_status = self._queue.get_hu_status(hu_item.text())

            if hu_status in ("processing", "ok", "duplicate"):
                action_disabled = menu.addAction("⛔ HU ya procesado — no se puede borrar")
                action_disabled.setEnabled(False)
                delete_action = None
            else:
                delete_action = menu.addAction("🗑 Borrar HU")
    ...
    def _delete_pallet(self, pallet_id: int):
        ...
        # 3. Reconstruir AMBOS índices después de borrar
        self._pallet_sep_rows.pop(pallet_id, None)
        self._update_pallet_sep_rows_indices()   # reconstruye _pallet_sep_rows
        self._rebuild_rows_index()               # reconstruye _rows
    ...
    def _delete_hu_row(self, row: int):
        ...
        # Reconstruir ambos índices después de borrar
            ...
            if self._queue.pallet_is_empty(pallet_id):
                if pallet_id == self._last_ui_pallet:
                    ...
                    if pallet_id in self._pallet_sep_rows:
                        ...
                        self._pallet_sep_rows.pop(pallet_id, None)
                        self._update_pallet_sep_rows_indices()
                        # Reconstruir de nuevo tras el segundo removeRow
                        self._rebuild_rows_index()
                    self._queue.release_pallet(pallet_id)
    ...
    def _rebuild_rows_index(self):
        """
        Reconstruye self._rows escaneando la columna 2 de la tabla.
        Debe llamarse después de cualquier operación que cambie el número de filas
        (removeRow, insertRow) para evitar índices stale.
        """
        self._rows.clear()
        for i in range(self.table.rowCount()):
            item = self.table.item(i, 2)
            if item and item.text():
                self._rows[item.text()] = i

    def _find_row_for_hu(self, hu_code: str) -> int | None:
        """
        Busca la fila de un HU escaneando la columna 2.
        Más lento que _rows, pero siempre correcto.
        Actualiza _rows como efecto secundario si encuentra la fila.
        """
        for i in range(self.table.rowCount()):
            item = self.table.item(i, 2)
            if item and item.text() == hu_code:
                self._rows[hu_code] = i   # reparar el índice stale en el momento
                return i
        return None
    ...
    def _on_item(self, item: HUItem):
        ...
        # Si el índice almacenado no apunta al HU correcto, buscar de verdad
        if r is None or self.table.item(r, 2) is None or self.table.item(r, 2).text() != item.hu_code:
            r = self._find_row_for_hu(item.hu_code)

        if r is None:
            return   # HU aún no tiene fila (no debería ocurrir, pero es seguro)
        ...
        elif item.status in ("error", "hu_not_found"):
        ...
        elif item.status in ("error", "hu_not_found") and not item.phase2_msg:
            # Error en F1 o HU no existe — F2 nunca se ejecutó
    ...
    def _refresh_kpis(self, s: dict):
        ...
        # Actualizar estado de botones Iniciar y Reprocesar
        self._update_action_buttons()
    ...
    def _set_running(self, r: bool):
        ...
        # Actualizar botones de inicio y reprocesamiento
        self._update_action_buttons()

    def _update_action_buttons(self):
        """
        Actualiza estado de btn_start y btn_reprocess según el estado de la cola.
        - btn_start: activo solo si hay HU pendientes y worker no está corriendo
        - btn_reprocess: activo solo si proceso completó y no hay pendientes
        """
        is_running = self._worker.is_running()
        stats = self._queue.stats
        has_pending = stats["pending"] > 0
        has_processed = stats["ok"] > 0 or stats["errors"] > 0
        
        # btn_start: activar si hay pendientes y no está corriendo
        self._footer.btn_start.setEnabled(has_pending and not is_running)
        
        # btn_reprocess: activar solo si proceso completó (no hay pendientes, hay items procesados)
        # y el worker no está corriendo
        can_reprocess = (not has_pending and has_processed and not is_running and stats["total"] > 0)
        self._footer.btn_reprocess.setEnabled(can_reprocess)

    def _update_start_button(self):
        """
        [Deprecated] Usa _update_action_buttons() en su lugar.
    ...
    def _reprocess(self):
        """Resetea todos los HUs procesados al estado 'pending' para reprocesarlos."""
        # Inhabilitar botón inmediatamente para evitar clicks múltiples
        self._footer.btn_reprocess.setEnabled(False)
        
        if self._worker.is_running():
            QMessageBox.warning(self, "Proceso activo",
                                "Detén el proceso antes de reprocesar.")
            # Re-evaluar estado del botón si hay error
            self._update_action_buttons()
            return
        if QMessageBox.question(
            self, "Reprocesar HUs",
            "Esto marcará todos los HUs procesados como 'Pendientes' para reprocesar.\n¿Continuar?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        ) == QMessageBox.StandardButton.No:
            # Usuario canceló - re-evaluar estado del botón
            self._update_action_buttons()
            return

        # Resetear todos los HUs en el modelo (incluyendo tiempos de pallet)
        count = self._queue.reprocess_all()
        
        # Actualizar cada fila de la tabla
        all_items = {item.hu_code: item for item in self._queue.get_all_items()}
        
        for row in range(self.table.rowCount()):
            # Buscar si es un separador de pallet (columna 0 tiene QLabel centrado)
            cell_widget = self.table.cellWidget(row, 0)
            if cell_widget and isinstance(cell_widget, QLabel):
                # Es un separador - buscar el pallet_id correspondiente
                for pallet_id, sep_row in self._pallet_sep_rows.items():
                    if sep_row == row:
                        # Actualizar el separador
                        self._update_pallet_separator(pallet_id)
                        break
                continue
            
            # Es una fila de HU normal - actualizar estado visual
            hu_code_item = self.table.item(row, 2)
            if hu_code_item:
                hu_code = hu_code_item.text()
                if hu_code in all_items:
                    item = all_items[hu_code]
                    bg, fg = status_colors(item.status)
                    # Actualizar celdas F1 y F2 con nuevos estados
                    self._cell(row, 3, item.f1_display, fg, bg)
                    self._cell(row, 4, item.f2_display, fg, bg)
        
        # Actualizar estadísticas
        stats = self._queue.stats
        self._refresh_kpis(stats)
        
        # Actualizar progreso y footer
        self._prog.set_mode(
            ProgressCard.MODE_IDLE, 
            f"Reprocessados {count} HUs. Listos para ser procesados nuevamente."
        )
        self._footer.set_status(f"↻ {count} HUs reiniciados para reprocesar.", C.BLUE)
        self.scan_input.setFocus()
    ...
    def _start_tour(self):
        ...
        tour_steps = [
            ...
            {
                'widget': self._footer.btn_reprocess,
                'title': '9. Reprocesar HUs',
                'description': 'Marca todos los HUs procesados como pendientes para reprocesarlos nuevamente.',
                'arrow_from': 'top'
            },
            {
                'widget': self._footer.btn_clear,
                'title': '10. Limpiar HUs',
                'description': 'Borra todas las HUs de la cola y limpia la tabla. Usa esto para comenzar de cero.',
                'arrow_from': 'top'
            },
            {
                'widget': self._shell.ham_btn,
                'title': '11. Menú principal',
